...
class AutorDAO:
    ...
    def listar(self) -> list[Autor]:
        ...
        for documento in db.autores.find():
            email = documento.get('email', 'default@email.com')
            telefone = documento.get('telefone', '123456789')
            bio = documento.get('bio', 'Autor de livros')
            aut = Autor(documento['nome'], email, telefone, bio)
