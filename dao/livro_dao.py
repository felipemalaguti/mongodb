...
class LivroDAO:
    ...
    def listar(self) -> list[Livro]:
        ...
        for documento in db.livros.find():
            ...
            liv = Livro(documento['titulo'], documento['resumo'], int(documento['ano']), int(documento['paginas']), documento['isbn'], categoria, editora, autor)
    ...
    def adicionar(self, livro: Livro) -> None:
        ...
        db.livros.insert_one({
            'titulo': livro.titulo, 'resumo': livro.resumo, 'ano': livro.ano, 'paginas': livro.paginas, 'isbn': livro.isbn,
            'codigo': livro.codigo, 'categoria_id': livro.categoria.id, 'editora_id': livro.editora.id, 'autor_id': livro.autor.id
        })
        client.close()
    ...
    def buscar_por_id(self, livro_id: str) -> Livro:
        ...
        if (resultado):
            ...
            liv = Livro(resultado['titulo'], resultado['resumo'], int(resultado['ano']), int(resultado['paginas']), resultado['isbn'], categoria, editora, autor)
